...
def move_is_valid(player_order, move, board):
    ...
    if (
        start[0] < 0
        or start[0] >= board.shape[0]
        or start[1] < 0
        or start[1] >= board.shape[1]
    ):
        # print("boundary 1")
        return False

    #   Check boundary condition
    if end[0] < 0 or end[0] >= board.shape[0] or end[1] < 0 or end[1] >= board.shape[1]:
        # print("boundary 2")
        return False
